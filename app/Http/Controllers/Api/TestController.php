...
//         $q = Test::query()->where('tenant_id', $tenantId);

//         if ($search = $request->get('search')) {
//             $q->where('name', 'like', "%{$search}%");
//         }

//         $tests = $q->orderBy('last_modified', 'desc')->get();
...
//                             // if q.id provided try update item otherwise create new
//                             if (!empty($q['id'])) {
...
//                             // choices (if provided) — replace existing choices for the item
//                             if (isset($q['choices']) && is_array($q['choices'])) {
//                                 // remove old choices that aren't provided (simpler: delete all and recreate)
...
//             return $this->success($test->load('sections.items.choices'), 'Test created', 201);
//         } catch (Exception $e) {
//             DB::rollBack();
//             return $this->error('Create failed: '.$e->getMessage(), 500);
//         }
//     }
...
//     {
//         $tenantId = $request->user()->tenant_id;
//         $test = Test::with('sections')->where('tenant_id', $tenantId)->findOrFail($id);
...
//             return $this->success($test->load('sections.items.choices'), 'Test updated');
//         } catch (Exception $e) {
//             DB::rollBack();
...
//     {
//         $tenantId = $request->user()->tenant_id;
//         $test = Test::where('tenant_id', $tenantId)->findOrFail($id);
//         $test->delete();
//         return response()->json(['status' => true, 'message' => 'Test deleted.']);
//     }
...
//         $tenantId = $request->user()->tenant_id;
//         $test = Test::where('tenant_id', $tenantId)->findOrFail($id);
//         $test->update(['status' => 'published', 'last_modified' => now()]);
//         return response()->json(['status' => true, 'data' => $test, 'message' => 'Published']);
//     }
...
//     {
//         $tenantId = $request->user()->tenant_id;
//         $orig = Test::with('sections')->where('tenant_id', $tenantId)->findOrFail($id);

//         DB::transaction(function () use ($orig, &$new) {
//             $new = $orig->replicate();
//             $new->name = $orig->name . ' (Copy)';
//             $new->status = 'draft';
//             $new->last_modified = now();
//             $new->push(); // save

//             // clone sections
//             foreach ($orig->sections as $section) {
//                 $s2 = $section->replicate();
//                 $s2->test_id = $new->id;
//                 $s2->push();
//             }
//         });

//         return response()->json(['status' => true, 'data' => $new, 'message' => 'Cloned']);
//     }
// }
